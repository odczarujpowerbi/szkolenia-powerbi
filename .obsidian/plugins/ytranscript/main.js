...
function extractParamsFromPage(htmlContent) {
  debugLog("\u{1F50D} DEBUG: Looking for ytInitialData script tag...");
  debugLog("\u{1F4CA} DEBUG: HTML content length:", htmlContent.length, "characters");
  ...
  if (!ytInitialDataMatch) {
    ...
    for (let i = 0; i < altPatterns.length; i++) {
      ...
      if (match) {
        debugLog(`\u2705 DEBUG: Found ytInitialData using alternative pattern ${i + 1}`);
  ...
  try {
    let findGetTranscriptEndpoint = function(obj, path = "", depth = 0) {
      ...
      if (depth === 0) {
        debugLog("\u{1F50D} DEBUG: Starting recursive search for getTranscriptEndpoint...");
      }
      if (obj.getTranscriptEndpoint && obj.getTranscriptEndpoint.params) {
        debugLog(`\u2705 DEBUG: Found getTranscriptEndpoint.params at path: ${path} (depth: ${depth})`);
        debugLog(`\u{1F4CF} DEBUG: Params length: ${obj.getTranscriptEndpoint.params.length} characters`);
        return obj.getTranscriptEndpoint.params;
      }
      if (depth < 3) {
        for (const key of Object.keys(obj)) {
          if (key.toLowerCase().includes("transcript") || key.toLowerCase().includes("engagement")) {
            debugLog(`\u{1F50D} DEBUG: Found transcript/engagement-related key: "${key}" at path: ${path} (depth: ${depth})`);
          }
        }
      }
    ...
    debugLog("\u{1F527} DEBUG: Parsing ytInitialData JSON (length:", ytInitialDataString.length, "chars)");
    let ytInitialData;
    try {
    ...
    if (params && typeof params === "string" && params.length > 50) {
      debugLog(`\u2705 DEBUG: Found valid getTranscriptEndpoint params (${params.length} chars):`, params.substring(0, 50) + "...");
      return params;
    } else if (params) {
      debugLog(`\u26A0\uFE0F DEBUG: Found getTranscriptEndpoint params but too short (${params.length} chars):`, params);
    } else {
      debugLog("\u274C DEBUG: No getTranscriptEndpoint.params found in ytInitialData");
    }
  } catch (error) {
...
function extractVisitorData(htmlContent2) {
  const visitorDataMatch = htmlContent2.match(/"visitorData"\s*:\s*"([^"]+)"/) || htmlContent2.match(/visitorData['"]\s*:\s*['"]([^"']+)['"]/);
  if (visitorDataMatch) {
    debugLog("\u2705 DEBUG: Found visitorData:", visitorDataMatch[1].substring(0, 20) + "...");
...
function parseVideoPageWithFallbacks(htmlContent2, config) {
  const parsedBody = (0, import_node_html_parser.parse)(htmlContent2);
  const titleMatch = htmlContent2.match(YOUTUBE_TITLE_REGEX);
  let title = "";
  if (titleMatch)
    title = titleMatch[1];
  const videoIdMatch = htmlContent2.match(YOUTUBE_VIDEOID_REGEX);
  let videoId = "";
  if (videoIdMatch)
    videoId = videoIdMatch[1].split("?v=")[1];
  const visitorData = extractVisitorData(htmlContent2);
  const pageParams = extractParamsFromPage(htmlContent2);
  debugLog("\u{1F50D} DEBUG: Params found on video page:", pageParams || "NOT FOUND");
  if (pageParams) {
    debugLog("\u{1F4CF} DEBUG: Page params length:", pageParams.length, "characters");
    debugLog("\u{1F50D} DEBUG: Page params preview:", pageParams.substring(0, 100) + "...");
  }
  const generatedParams = generateAlternativeTranscriptParams(videoId, (config == null ? void 0 : config.lang) || "en");
  debugLog("\u{1F527} DEBUG: Generated", generatedParams.length, "params variations for video ID:", videoId);
  generatedParams.forEach((params2, index) => {
    debugLog(`  Generated variation ${index + 1} (${params2.length} chars):`, params2.substring(0, 50) + "...");
  });
  if (pageParams) {
    let foundMatch = false;
    generatedParams.forEach((generatedParam, index) => {
      if (generatedParam === pageParams) {
        debugLog(`\u{1F3AF} MATCH! Page params identical to generated variation ${index + 1}`);
        foundMatch = true;
      }
    });
    if (!foundMatch) {
      debugLog("\u{1F504} DEBUG: Page params are DIFFERENT from all generated variations - this could be video-specific!");
    }
  }
  const allParams = pageParams ? [pageParams, ...generatedParams] : generatedParams;
  if (pageParams) {
    debugLog("\u2705 DEBUG: Will try page params FIRST (attempt 1), then", generatedParams.length, "generated variations");
  } else {
    debugLog("\u26A0\uFE0F DEBUG: No valid page params found, will try", generatedParams.length, "generated variations only");
  }
  const transcriptRequests = allParams.map((params2) => {
    const requestBody = {
      context: {
        client: {
          clientName: "WEB",
