## 2: ALL i REMOVEFILTERS - usuwanie filtrów
...
V1 = CALCULATE([Total Sales], ALL(dProduct[ProductCategoryName]))

// Usuwa WSZYSTKIE filtry z tabeli dProduct
// Kolor, marka, podkategoria - wszystko zignorowane
V2 = CALCULATE([Total Sales], ALL(dProduct))
```
...
V1 = CALCULATE([Total Sales], ALL(dProduct))
V2 = CALCULATE([Total Sales], REMOVEFILTERS(dProduct))
```
...
## 3: ALLEXCEPT - usuń wszystko OPRÓCZ
...
### Problem, który rozwiązuje ALLSELECTED
...
V1 = CALCULATE([Total Sales], ALLSELECTED(dProduct[ProductCategoryName]))

// Usuwa filtr wizualizacji ze wszystkiego (produkty, czas, geografia...)
V2 = CALCULATE([Total Sales], ALLSELECTED())
```
...
- ALLSELECTED daje sumę dla "Computers" (400k) - respektuje slicer

### Ćwiczenie 5

**Zadanie 1:** Napisz miarę pokazującą udział każdego produktu w sprzedaży wybranych w slicerze kategorii. Użyj ALLSELECTED.

**Zadanie 2:** Masz raport ze slicerem roku (wybrane: 2023, 2024) i tabelą pokazującą miesiące. Napisz miarę "% of Selected Period", która pokaże udział każdego miesiąca w sumie za wybrane lata. Co się stanie, gdy użytkownik nie wybierze nic w slicerze?
## 4: KEEPFILTERS - zachowaj istniejące filtry
