createOrReplace
    ...
    table _HTML
        lineageTag: bb18f40c-b30c-4167-a08c-3bd7217e6395
...
"
```
            lineageTag: 5efe1516-a3e5-418b-8835-a468d25039fe
...
"
```
            lineageTag: 8356ba52-00de-4798-812c-1173f2217329
...
"
```
            lineageTag: 7b8b0415-8392-4179-94fc-a10a9e48137c
...
"
```
            lineageTag: 302e5652-fb54-43d9-b0cc-bb4612407f38
...
"
```
            lineageTag: 0bb1a41f-712a-40eb-8720-ab310b81f983
...
"
```
            lineageTag: a258cb74-0771-4854-ae3a-fb422995bacd
...
"
```
            lineageTag: b6cd042f-0325-4481-8e5e-a189aec43985
...
<div class='pagination'>
    ...
    <span class='page-info'>Zadanie <span id='currentPage'>1</span> z <span id='totalPages'>5</span></span>
...
<script>
    let currentPage = 1;
    const totalPages = 5;
    const slotsPerPage = [1, 1, 1, 3, 1];
    let slots = new Array(slotsPerPage[0]).fill('');
    const correctSolutions = [['Sprzedaz[Kwota]'], ['\u2035Tabela Sprzeda\u017cy\u2035[Kwota Netto]'], ['SUM'], ['IF', 'Wysoka', 'Niska'], ['&']];
...
"
```
            lineageTag: da55b7f8-880a-445c-a041-32d174163139
...
"
```
            lineageTag: 6114b0a4-4a7b-4963-9d36-5bfdba28c1d9
...
"
```
            lineageTag: b357573b-73f0-4476-975e-a7d558974c21
...
"
```
            lineageTag: a5e0aafa-7c4b-4e3b-86df-c2c9f9df4400
...
"
```
            lineageTag: 1b87c94a-098c-44d4-95ae-cb0775eee1ce
...
"
```
            lineageTag: 7efd68d8-650f-489a-9bec-072e5cbca0f5
...
"
```
            lineageTag: c0639b17-0d5c-42c5-9def-4db48528ef64
...
"
```
            lineageTag: 2b28bc38-6097-4643-81ce-10acc19b6d92
...
"
```
            lineageTag: 432a921c-d067-484e-8468-f9da6f474e72
...
<div class='container'>
    ...
    <div class='page'>
        ...
        <div class='answers'>
            ...
            <div class='answer-option' onclick='selectAnswer(3, 1)'>
                B) Tak, ale zwróci tę samą wartość dla wszystkich wierszy
            </div>
            <div class='answer-option' onclick='selectAnswer(3, 2)'>
    ...
    <div class='page'>
        <h1>Pytanie 7</h1>
        <div class='question-box'>
            <p><strong>Co jest lepszym wyborem dla obliczeń, które muszą jedynie reagować na filtry użytkownika - nie służą do filtrowania?</strong></p>
        </div>
        <div class='answers'>
...
<script>
    ...
    const explanations = [
        ...
        'Funkcje agregujące jak SUM() MOŻNA użyć w kolumnach kalkulowanych, ale zwrócą one wartość zagregowaną dla CAŁEJ tabeli (tę samą dla każdego wiersza), a nie wartość dla bieżącego wiersza. W kontekście wiersza (row context) zazwyczaj odwołujesz się bezpośrednio do kolumn: `Sales[Quantity] * Sales[Price]`. Jeśli potrzebujesz agregacji z iteracją po wierszach, użyj funkcji X (SUMX, AVERAGEX) lub CALCULATE dla transformacji kontekstu',
...
"
```
            lineageTag: 64152f76-f785-48ac-9a4b-dc0ab4b75470
...
<div class='container' id='viz_b9c59316'>
    ...
    <div class='page'>
        ...
        <span class='dax-function'>CALCULATE</span>(
            [Sprzedaż Netto],
            dKalendarz[Dzien] &gt;= <span class='dax-number'>1</span> &&
            dKalendarz[Dzien] &lt;= <span class='dax-number'>5</span>
        )</code></pre>
    ...
    <div class='page'>
        ...
        <span class='dax-function'>CALCULATE</span>(
            [Sprzedaż Netto],
            dKalendarz[Dzien] = <span class='dax-number'>6</span> ||
            dKalendarz[Dzien] = <span class='dax-number'>7</span>
        )</code></pre>
...
"
```
            lineageTag: 88bcabd7-0ea2-4cdf-8f11-95e6de3a4f88
...
"
```
            lineageTag: b39c12e7-9fc2-43d1-9de4-5f152e60e466
...
"
```
            lineageTag: 0107de9e-d559-4dca-807c-1626d7d755aa
...
"
```
            lineageTag: b8eaa11a-836f-41fd-ac09-8999171ca7b9
...
<div class='container'>
    <div class='task-description'>
        ...
        Chcesz policzyć sprzedaż dla kategorii NAPOJE lub SŁODYCZE. Jak  zapisać to najlepiej?
    </div>
    ...
    <div>
        ...
        <span class='function-chip' draggable='true' ondragstart='drag(event)' data-function='dKategorie[Kategoria] IN {″NAPOJE″ˏ ″SŁODYCZE″}'>dKategorie[Kategoria] IN {″NAPOJE″ˏ ″SŁODYCZE″}</span>
        <span class='function-chip' draggable='true' ondragstart='drag(event)' data-function='dKategorie[Kategoria] = {NAPOJEˏ SŁODYCZE}'>dKategorie[Kategoria] = {NAPOJEˏ SŁODYCZE}</span>
        ...
        <span class='function-chip' draggable='true' ondragstart='drag(event)' data-function='OR()'>OR()</span>
    </div>
...
<script>
    let currentPage = 1;
    const totalPages = 3;
    const slotsPerPage = [3, 1, 2];
    let slots = new Array(slotsPerPage[0]).fill('');
    const correctSolutions = [['CALCULATE', '[Sprzeda\u017c Netto]', 'dKategorie[Kategoria]'], ['REMOVEFILTERS'], ['&&', '||']];
...
"
```
            lineageTag: 7428284c-23be-4d83-8f6e-befbd2ae4a3c
...
"
```
            lineageTag: a70778a4-1447-40e7-a0ab-2c48c2471533
...
<div class='container' id='viz_624e3a9b'>
    ...
    <div class='page active'>
        ...
        <p>Użytkownik wybiera w slicerze kategorię 'NAPOJE'. Chcesz pokazać:</p>
        <ul>
        <li>Sprzedaż NAPOJE: 400 000 zł</li>
        <li>Sprzedaż całkowita: 1 000 000 zł (mimo filtru!)</li>
        <li>Udział: 40%</li>
        </ul>
        <pre><code>DAX
        Sprzedaż Netto = <span class='dax-function'>SUM</span>(ffSprzedaż[fSprzedażAmount])

        Sprzedaż Netto All Categories = 
        <span class='dax-function'>CALCULATE</span>(
            [Sprzedaż Netto],
            <span class='dax-function'>ALL</span>(dKategorie[Kategoria])
        )

        fSprzedaż % = <span class='dax-function'>DIVIDE</span>([Sprzedaż Netto], [Sprzedaż Netto All Categories])</code></pre>
        <p>Co się dzieje:</p>
        <table>
          <thead>
            <tr>
              ...
              <th>[Sprzedaż Netto All Categories]</th>
            </tr>
          </thead>
          <tbody>
            <tr>
        ...
        <p><code>ALL(dKategorie[Kategoria])</code> mówi: 'usuń filtr z kategorii, ale zostaw inne filtry (np. na roku, kraju)'.</p>
    ...
    <div class='page'>
        ...
        <ul>
        <li> Usuwa <strong>WSZYSTKIE</strong> filtry z tabeli <strong>dKategorie</strong></li>
        <li> Kolor, marka, podKategorie - wszystko zignorowane</li>
        </ul>
        <pre><code>DAX
        <span class='dax-function'>CALCULATE</span>(
        	[Sprzedaż Netto],
        	 <span class='dax-function'>ALL</span>(dKategorie)
         )</code></pre>
    ...
    <div class='page'>
        ...
        <span class='dax-function'>CALCULATE</span>(
        	[Sprzedaż Netto],
        	 <span class='dax-function'>REMOVEFILTERS</span>(dKategorie)
        )</code></pre>
        ...
        All Products Table = <span class='dax-function'>ALL</span>(dKategorie)

        // ALL jako modyfikator w <span class='dax-function'>CALCULATE</span> (usuwa filtry)
        fSprzedaż Without Filter = <span class='dax-function'>CALCULATE</span>([Sprzedaż Netto], <span class='dax-function'>ALL</span>(dKategorie))

        // REMOVEFILTERS - jednoznaczne, tylko modyfikator
        fSprzedaż Without Filter = <span class='dax-function'>CALCULATE</span>([Sprzedaż Netto], <span class='dax-function'>REMOVEFILTERS</span>(dKategorie))</code></pre>
...
"
```
            lineageTag: ec880ed7-fa50-4d1d-9a1d-7fb4fe62b761
...
<div class='container' id='viz_c3f109de'>
    ...
    <div class='page active'>
        ...
        <p>Wyobraź sobie tabelę dKategorie z kolumnami: ProductKey, ProductName, Kategoria, ProductSubcategoryName, BrandName, Color, UnitPrice.</p>
        ...
        <span class='dax-function'>CALCULATE</span>(
            [Sprzedaż Netto],
            <span class='dax-function'>ALLEXCEPT</span>(dKategorie, dKategorie[Kategoria])
        )</code></pre>
        <p><code>ALLEXCEPT</code> mówi: 'usuń wszystkie filtry z dKategorie OPRÓCZ filtra na Kategoria'.</p>
    ...
    <div class='page'>
        ...
        <p>Raport pokazuje produkty w kategorii 'NAPOJE':</p>
        <table>
          <thead>
            <tr>
              <th>Produkt</th>
        ...
        <p>Gdy użytkownik przełączy na kategorię 'NAPOJE':</p>
        <table>
          <thead>
            <tr>
              <th>Produkt</th>
...
"
```
            lineageTag: 0a84f523-fd5b-4da3-8708-c6e8d4946be6
...
<div class='container' id='viz_adeb3b9a'>
    ...
    <!-- Strona 1 -->
    <div class='page active'>
        <p>Masz raport z tabelą pokazującą sprzedaż po kategoriach. Użytkownik wybiera w slicerze trzy kategorie: NAPOJE, WINA, SPOŻYWKA. Chce widzieć udział każdej z tych trzech w ich łącznej sumie.</p>
        <pre><code>DAX
        // Próba z ALL
        fSprzedaż % with ALL =
        <span class='dax-function'>DIVIDE</span>(
            [Sprzedaż Netto],
            <span class='dax-function'>CALCULATE</span>([Sprzedaż Netto], <span class='dax-function'>ALL</span>(dKategorie[Kategoria]))
        )</code></pre>
        <p>Problem: ALL usuwa WSZYSTKIE filtry z kategorii - czyli ignoruje też slicer. Mianownik to sprzedaż wszystkich kategorii w danych, nie tylko trzech wybranych.</p>
        <table>
          <thead>
            <tr>
              <th>Kategoria</th>
    ...
    <div class='page'>
        ...
        <span class='dax-function'>DIVIDE</span>(
            [Sprzedaż Netto],
            <span class='dax-function'>CALCULATE</span>([Sprzedaż Netto], <span class='dax-function'>ALLSELECTED</span>(dKategorie[Kategoria]))
        )</code></pre>
        ...
        <span class='dax-function'>CALCULATE</span>(
        	[Sprzedaż Netto],
        	 <span class='dax-function'>ALLSELECTED</span>(dKategorie[Kategoria])
         )</code></pre>
        <pre><code>DAX
    ...
    <div class='page'>
        ...
        Grand Total ALL =
        <span class='dax-function'>CALCULATE</span>([Sprzedaż Netto], <span class='dax-function'>ALL</span>(dKategorie))

        // Ignoruje wizualizację, respektuje slicery
        Grand Total ALLSELECTED =
        <span class='dax-function'>CALCULATE</span>([Sprzedaż Netto], <span class='dax-function'>ALLSELECTED</span>(dKategorie))</code></pre>
        <p>Scenariusz: Slicer = 'NAPOJE', tabela pokazuje podkategorie:</p>
        <table>
          <thead>
            <tr>
              <th>PodKategorie</th>
        ...
        <li>ALLSELECTED daje sumę dla 'NAPOJE' (400k) - respektuje fragmentator</li>
        </ul>
        <p>---</p>
...
"
```
            lineageTag: 16530cf6-aa67-4890-9cc7-b00de02531ca
...
"
```
            lineageTag: a02555cb-17a7-4205-bdd7-07979fc308d0
...
<div class='container' id='viz_de834375'>
    ...
    <div class='page active'>
        ...
        <p>Co się stanie, gdy użytkownik wybierze w slicerze 'WINA'?</p>
        <table>
          <thead>
            <tr>
              <th>Kontekst użytkownika</th>
              <th>[Sprzedaż Netto]</th>
              <th>[fSprzedaż NAPOJE]</th>
            </tr>
          </thead>
          <tbody>
            <tr>
    ...
    <div class='page'>
        ...
        <pre><code>DAX
        fSprzedaż NAPOJE Keep =
        <span class='dax-function'>CALCULATE</span>(
            [Sprzedaż Netto],
            <span class='dax-function'>KEEPFILTERS</span>(dKategorie[Kategoria] = 'NAPOJE')
        )</code></pre>
        <p>Teraz:</p>
        <table>
          <thead>
        ...
        <p>Gdy użytkownik wybiera 'WINA', KEEPFILTERS szuka przecięcia: (WINA) ∩ (NAPOJE) = zbiór pusty = BLANK.</p>
...
"
```
            lineageTag: b4d1674f-96c7-4c01-a45a-00ea850a9afe
...
"
```
            lineageTag: 4da2fd62-1200-4431-abae-8d348c01b60f
...
<div class='container'>
    <div class='task-description'>
        ...
        Masz miarę ze stałym filtrem NAPOJE. Chcesz, żeby gdy użytkownik wybierze NAPOJE w slicerze, miara zwróciła BLANK (bo nie ma przecięcia). Jakiej funkcji użyć?
    </div>
...
"
```
            lineageTag: 235e160b-a3ad-471c-8d1d-6bf18cd14287
...
"
```
            lineageTag: 1c3006cd-e412-4de4-b2fd-f0ee36af093b
...
"
```
            lineageTag: b8f285b5-6151-496e-a3e3-095994da5b98
...
"
```
            lineageTag: b56e9606-652f-415c-b386-260eaf250325
...
<div class='container'>
    <div class='task-description'>
        ...
        Jak prawidłowo zdefiniować tymczasową miarę w Query View?
    </div>
...
"
```
            lineageTag: 9ddf6f49-2167-4456-b5f3-9a467ba9b1e2
...
<div class='container'>
    ...
    <div class='page'>
        <h1>Pytanie 2</h1>
        <div class='question-box'>
            <p><strong>Ile razy może wystąpić słowo `RETURN` w jednym wyrażeniu DAX?</strong></p>
        </div>
        <div class='answers'>
            <div class='answer-option' onclick='selectAnswer(1, 0)'>
...
<script>
    ...
    const correctAnswers = [1, 1, 1, 1, 1, 2];
    ...
    const explanations = [
        ...
        'W jednym wyrażeniu może być tylko jedno `RETURN`, które zwraca końcowy wynik. Miara z kolei może mieć wiele `RETURN`, zagnieżdzonych w funkcjach, np. w SUMX',
...
"
```
            lineageTag: ab02cc55-8752-4415-ace9-4e3c015b326e
...
"
```
            lineageTag: e835b011-cfe2-4100-a7bd-85c74ca5a79e
...
"
```
            lineageTag: 518b6dfb-512b-4cd4-ae99-35f8db5c8497
...
"
```
            lineageTag: 53dd4e23-159b-4b36-a253-f88c6e22bb9b
...
"
```
            lineageTag: 57254fa5-41bb-426c-a640-4f9fd068ba4e
...
"
```
            lineageTag: b4a6bc01-1a7d-4621-bfa6-3eedc5020ee9
...
"
```
            lineageTag: 735256da-0456-4fdd-93d7-29f39bdb1208
...
"
```
            lineageTag: b29794e1-d128-4ad3-83d0-5d85005fcddb
...
"
```
            lineageTag: 2e560adf-9ace-4123-b26d-0bd3e4e08bdf
...
<div class='container'>
    ...
    <div>
        ...
        <span class='function-chip' draggable='true' ondragstart='drag(event)' data-function='SUMX'>SUMX</span>
        <span class='function-chip' draggable='true' ondragstart='drag(event)' data-function='CALCULATE'>CALCULATE</span>
        <span class='function-chip' draggable='true' ondragstart='drag(event)' data-function='DIVIDE'>DIVIDE</span>
        <span class='function-chip' draggable='true' ondragstart='drag(event)' data-function='BLANK'>BLANK</span>
    </div>
    ...
    <div class='code-container'>
        ...
        <div>    <div class='drop-zone' data-slot='0' ondrop='drop(event)' ondragover='allowDrop(event)' ondragleave='dragLeave(event)'></div>(</div>
        <div>        Sprzedaz,</div>
        <div>        <div class='drop-zone' data-slot='1' ondrop='drop(event)' ondragover='allowDrop(event)' ondragleave='dragLeave(event)'></div>(Sprzedaz[Wartosc Netto] - Sprzedaz[Koszty Netto], Sprzedaz[Wartosc Netto], <div class='drop-zone' data-slot='2' ondrop='drop(event)' ondragover='allowDrop(event)' ondragleave='dragLeave(event)'></div>())</div>
        <div>    )</div>
    </div>
...
<script>
    let currentPage = 1;
    const totalPages = 3;
    const slotsPerPage = [1, 3, 2];
    let slots = new Array(slotsPerPage[0]).fill('');
    const correctSolutions = [['SUMX'], ['AVERAGEX', 'DIVIDE', 'BLANK'], ['COUNTX', 'Sprzedaz']];
...
"
```
            lineageTag: b867c758-2caa-436a-b9fa-42786df4051a
...
<div class='container'>
    ...
    <div class='page'>
        ...
        <div class='answers'>
            ...
            <div class='answer-option' onclick='selectAnswer(3, 1)'>
                B) Gdy musisz dokonać operacji matematycznych (lub tekstowych) w ramach pojedynczego wiersza, a nie całej kolumny
            </div>
            <div class='answer-option' onclick='selectAnswer(3, 2)'>
...
"
```
            lineageTag: 1f98871b-eb42-4f3e-b37a-8dddb779c640
...
"
```
            lineageTag: 0b45d1d9-a237-43cd-a567-10dcc356529b
...
"
```
            lineageTag: 172c5ddb-2ae3-4945-ab64-8c9c6043c4f2
...
"
```
            lineageTag: c8b5bc63-e943-458d-9519-29e84dc4aa45
...
"
```
            lineageTag: 5c5c93bc-7d70-4689-8ac9-80c7482866a8
...
"
```
            lineageTag: 86f14756-d3bb-4280-a46f-9d7cab31db3b
...
"
```
            lineageTag: ee1941b3-fa67-468c-8ceb-10af3d050cff
...
"
```
            lineageTag: 39a03c49-98da-43a4-9441-8572ef1412e4
...
<head>
    <meta charset='UTF-8'>
    <meta name='viewport' content='width=device-width, initial-scale=1.0'>
    <title>Funkcje Time Intelligence w DAX</title>
    <style>
    * {
                margin: 0;
                padding: 0;
...
<div class='container'>
    ...
    <div class='navigation'>
        <button id='prevBtn' onclick='changePage(-1)'>← Poprzednia</button>
        <span class='page-indicator'>
            Pytanie <span id='currentPage'>1</span> z <span id='totalPages'>10</span>
        </span>
        <button id='nextBtn' onclick='changePage(1)'>Następna →</button>
    </div>
    ...
    <div class='page active'>
        <h1>Pytanie 1</h1>
        <div class='question-box'>
            <p><strong>Co jest wymagane, aby funkcje Time Intelligence w DAX działały poprawnie?</strong></p>
        </div>
        <div class='answers'>
            <div class='answer-option' onclick='selectAnswer(0, 0)'>
    ...
    <div class='page'>
        <h1>Pytanie 2</h1>
        <div class='question-box'>
            <p><strong>Co zmieni dodanie `TOTALYTD` do funkcji `SUM` w `CALCULATE`?</strong></p>
        </div>
        <div class='answers'>
            <div class='answer-option' onclick='selectAnswer(1, 0)'>
    ...
    <div class='page'>
        <h1>Pytanie 3</h1>
        <div class='question-box'>
            <p><strong>Który wzór poprawnie obliczy sprzedaż z tego samego okresu rok temu?</strong></p>
        </div>
        <div class='answers'>
            <div class='answer-option' onclick='selectAnswer(2, 0)'>
    ...
    <div class='page'>
        <h1>Pytanie 4</h1>
        <div class='question-box'>
            <p><strong>Co zwróci miara `CALCULATE(SUM(Sprzedaz[Wartość]), DATESBETWEEN(Kalendarz[Data], DATE(2024,1,1), DATE(2024,3,31)))` gdy jest użyta w tabeli z podziałem na miesiące?</strong></p>
        </div>
        <div class='answers'>
            <div class='answer-option' onclick='selectAnswer(3, 0)'>
    ...
    <div class='page'>
        <h1>Pytanie 5</h1>
        <div class='question-box'>
            <p><strong>Co się stanie, gdy użyjesz `TOTALMTD(SUM(Sprzedaz[Wartość]), Kalendarz[Data])` w kontekście, gdzie wybrany jest koniec miesiąca?</strong></p>
        </div>
        <div class='answers'>
            <div class='answer-option' onclick='selectAnswer(4, 0)'>
    ...
    <div class='page'>
        <h1>Pytanie 6</h1>
        <div class='question-box'>
            <p><strong>Co zwróci funkcja `PARALLELPERIOD(Kalendarz[Data], -4, QUARTER)` gdy bieżący kontekst to Q3 2024?</strong></p>
        </div>
        <div class='answers'>
            <div class='answer-option' onclick='selectAnswer(5, 0)'>
                A) Q2 2024
    ...
    <div class='page'>
        <h1>Pytanie 7</h1>
        <div class='question-box'>
            <p><strong>Dlaczego wzór `SUM(Sprzedaz[Wartość]) / CALCULATE(SUM(Sprzedaz[Wartość]), ALL(Kalendarz))` może nie dać prawidłowego udziału procentowego w kontekście roku?</strong></p>
        </div>
        <div class='answers'>
            <div class='answer-option' onclick='selectAnswer(6, 0)'>
    ...
    <div class='page'>
        <h1>Pytanie 8</h1>
        <div class='question-box'>
            <p><strong>Jaka jest różnica między `DATEADD` a `PARALLELPERIOD`?</strong></p>
        </div>
        <div class='answers'>
        ...
        <button class='check-button' onclick='checkAnswer(7, 1)'>Sprawdź odpowiedź</button>
    ...
    <div class='page'>
        <h1>Pytanie 9</h1>
        <div class='question-box'>
            <p><strong>Co zrobi funkcja `FIRSTDATE` w kontekście tabeli z podziałem na miesiące?</strong></p>
        </div>
        <div class='answers'>
        ...
        <button class='check-button' onclick='checkAnswer(8, 1)'>Sprawdź odpowiedź</button>
    ...
    <div class='page'>
        <h1>Podsumowanie quizu</h1>
        <div class='score-box' id='finalScore'>
            Twój wynik: <span id='scoreText'>0/9</span> (<span id='percentText'>0%</span>)
        </div>
        <div id='summaryContent'></div>
        <button onclick='restartQuiz()' style='margin-top: 20px; width: 100%;'> Rozpocznij quiz od nowa</button>
    </div>
...
<script>
    ...
    const explanations = [
        'Funkcje Time Intelligence wymagają tabeli dat (dimension table) z nieprzerwanym ciągiem dat, która jest oznaczona jako tabela dat w modelu. Bez tego funkcje te nie będą działać poprawnie lub w ogóle.',
        '`TOTALYTD` to funkcja agregująca wartości od początku roku do daty określonej przez bieżący kontekst filtra. W przeciwieństwie do `SUM`, która sumuje wartości tylko z bieżącego kontekstu, `TOTALYTD` automatycznie modyfikuje kontekst, aby uwzględnić wszystkie daty od 1 stycznia do daty w kontekście.',
        '`SAMEPERIODLASTYEAR` automatycznie przesuwa kontekst dat o rok wstecz, zachowując ten sam okres (dzień, miesiąc lub dowolny inny zakres dat). Funkcja ta jest zaprojektowana specjalnie do porównań rok do roku i automatycznie obsługuje wszystkie przypadki brzegowe.',
        'Funkcja `DATESBETWEEN` w połączeniu z `CALCULATE` zastępuje kontekst filtra dat określonym zakresem. Oznacza to, że niezależnie od tego, który miesiąc jest w wierszu tabeli, miara zawsze zwróci sumę za cały Q1 2024, ponieważ `CALCULATE` nadpisuje kontekst filtra.',
        '`TOTALMTD` oblicza wartość skumulowaną od początku miesiąca. Gdy kontekst obejmuje cały rok, funkcja oblicza wartość do ostatniego dnia ostatniego miesiąca w tym roku, czyli faktycznie sumę za cały rok, ale przez pryzmat od początku miesiąca dla grudnia.',
        '`PARALLELPERIOD` przesuwa bieżący kontekst o określoną liczbę okresów wstecz lub w przód. W tym przypadku -4 kwartały od Q3 2024 to Q3 2023. Funkcja zwraca tabelę dat dla tego równoległego okresu, zachowując ten sam typ okresu (kwartał).',
        '`ALL(Kalendarz)` usuwa wszystkie filtry z tabeli Kalendarz, włączając w to filtry roku, kwartału i miesiąca. W efekcie mianownik zawsze pokazuje sumę globalną ze wszystkich lat, a nie tylko z bieżącego roku. Aby obliczyć udział w ramach roku, należałoby użyć `ALL(Kalendarz[Miesiąc])` lub `ALLEXCEPT(Kalendarz, Kalendarz[Rok])`.',
        '`DATEADD` przesuwa każdą datę w kontekście o określoną liczbę okresów (np. każdy dzień w marcu zostanie przesunięty o rok wstecz). `PARALLELPERIOD` natomiast przesuwa cały okres jako kompletną jednostkę (np. cały Q1 2024 zostanie przesunięty do Q1 2023), zachowując strukturę okresu. To powoduje różne wyniki, szczególnie przy porównaniach okresów.',
        '`FIRSTDATE` zwraca pierwszą datę z bieżącego kontekstu filtra. Gdy kontekstem jest pojedynczy miesiąc (np. marzec 2024), funkcja zwróci pierwszy dzień tego miesiąca (1 marca 2024). Jest to funkcja uwzględniająca kontekst, nie zwraca globalnie pierwszej daty.'
    ];
...
"
```
            lineageTag: e369bfec-3e26-4307-b92b-078bb8c5f34f
...
<div class='container' id='viz_f3f0fa1b'>
    <!-- Nawigacja na górze -->
    <div class='navigation'>
        <button id='prevBtn_f3f0fa1b' onclick='changePage_f3f0fa1b(-1)'>← Poprzednia</button>
        <span class='page-indicator'>
            Strona <span id='currentPage_f3f0fa1b'>1</span> z <span id='totalPages_f3f0fa1b'>2</span>
        </span>
        <button id='nextBtn_f3f0fa1b' onclick='changePage_f3f0fa1b(1)'>Następna →</button>
    </div>

    <!-- Strona 1 -->
    <div class='page active'>
        <h1>CALCULATEDGROUP</h1>
        <p>Alternatywne podejście do <code>USERELATIONSHIP</code> – zamiast tworzyć osobne miary dla każdego typu daty, tworzysz jedną calculation group, która dynamicznie przełącza aktywną relację w zależności od wyboru użytkownika.</p>
        <p>W Calculation Group tworzysz calculation items z wyrażeniami:</p>
        <pre><code><span class='dax-function'>CALCULATE</span>(
            <span class='dax-function'>SELECTEDMEASURE</span>(), <span class='dax-comment'>-- PowerBI automatycznie wstawi tutaj miarę!</span>
            <span class='dax-function'>USERELATIONSHIP</span>(´Sprzedaż´[Data Sprzedaży], dKalendarz[Date])
        )</code></pre>
    ...
    <!-- Strona 2 -->
    <div class='page'>
        <h2><strong>Problem biznesowy</strong></h2>
        <p>Identyczny jak przy <code>USERELATIONSHIP</code>: Tabela Sprzedaż ma wiele dat (Faktura, Sprzedaż, Dostawa), ale tylko jedno aktywne połączenie z Kalendarzem.</p>
        <p><strong>Krok 1</strong>: Utwórz Calculation Group o nazwie <strong>Kontekst Daty</strong></p>
        <p><strong>Krok 2</strong>: Dodaj calculation items:</p>
        <pre><code>wg Data Fakturowania =
        <span class='dax-function'>CALCULATE</span>(
            <span class='dax-function'>SELECTEDMEASURE</span>()
        )</code></pre>
        <pre><code>wg Data Sprzedaży =
        <span class='dax-function'>CALCULATE</span>(
            <span class='dax-function'>SELECTEDMEASURE</span>(),
            <span class='dax-function'>USERELATIONSHIP</span>(´Sprzedaż´[Data Sprzedaży], dKalendarz[Date])
        )</code></pre>
        <pre><code>wg Data Dostawy =
        <span class='dax-function'>CALCULATE</span>(
            <span class='dax-function'>SELECTEDMEASURE</span>(),
            <span class='dax-function'>USERELATIONSHIP</span>(´Sprzedaż´[Data Dostawy], dKalendarz[Date])
        )</code></pre>
...
(function() {
    ...
    window['showPage_f3f0fa1b'] = function(n) {
        if (n > totalPages) currentPage_f3f0fa1b = totalPages;
        if (n < 1) currentPage_f3f0fa1b = 1;

        for (var i = 0; i < pages.length; i++) {
            pages[i].classList.remove('active');
        }
        pages[currentPage_f3f0fa1b - 1].classList.add('active');

        document.getElementById('currentPage_' + vizId).textContent = currentPage_f3f0fa1b;
        document.getElementById('prevBtn_' + vizId).disabled = (currentPage_f3f0fa1b === 1);
        document.getElementById('nextBtn_' + vizId).disabled = (currentPage_f3f0fa1b === totalPages);
    };

    window['changePage_f3f0fa1b'] = function(n) {
        currentPage_f3f0fa1b += n;
        window['showPage_f3f0fa1b'](currentPage_f3f0fa1b);
    };

    // Inicjalizacja
    window['showPage_f3f0fa1b'](1);
...
"
```
            lineageTag: e088b0cc-cd62-4de9-9f62-4b5be7d7e04d
...
<head>
    <meta charset='UTF-8'>
    <meta name='viewport' content='width=device-width, initial-scale=1.0'>
    <title>Proporcjonalny podział celu</title>
    <style>
            * {
                margin: 0;
                padding: 0;
...
<div class='container' id='viz_68f25e0b'>
    <!-- Nawigacja na górze -->
    <div class='navigation'>
        <button id='prevBtn_68f25e0b' onclick='changePage_68f25e0b(-1)'>← Poprzednia</button>
        <span class='page-indicator'>
            Strona <span id='currentPage_68f25e0b'>1</span> z <span id='totalPages_68f25e0b'>2</span>
        </span>
        <button id='nextBtn_68f25e0b' onclick='changePage_68f25e0b(1)'>Następna →</button>
    </div>

    <!-- Strona 1 -->
    <div class='page active'>
        <h1>Proporcjonalny podział celu</h1>
        <p>Miara rozbija miesięczny cel na poszczególne dni, uwzględniając różną wagę dni roboczych i weekendów/świąt.</p>
        <h2><strong>Jak to działa?</strong></h2>
        <p><strong>Iteracja po datach</strong> - <code>SUMX</code> przechodzi przez każdą datę w bieżącym kontekście (dzień, tydzień, miesiąc - w zależności od wizualizacji)</p>
        <p>Dla każdej daty:</p>
        <ul>
        <li>Pobiera miesięczny cel z tabeli Targety</li>
        <li>Oblicza sumę wag wszystkich dni w tym miesiącu (dzień roboczy = waga 1, weekend/święto = waga 0.5)</li>
        <li>Sprawdza wagę bieżącego dnia (1 lub 0.5)</li>
        <li>Dzieli miesięczny cel przez sumę wag i mnoży przez wagę bieżącego dnia</li>
        </ul>
        <p><strong>Wynik</strong>:</p>
        <ul>
        <li>Na poziomie dnia: wartość proporcjonalna do wagi (dni robocze dostają 2x więcej niż weekendy)</li>
        <li>Na poziomie total: suma wartości dziennych = dokładnie 100% miesięcznego celu</li>
        </ul>
    ...
    <div class='page'>
        ...
        <ul>
        <li>Cel miesięczny: 30 000 zł</li>
        <li>Miesiąc ma 20 dni roboczych (waga 20) + 10 weekendów (waga 5) = suma wag 25</li>
        <li>Dzień roboczy dostanie: <code>30 000 / 25 × 1 = 1 200 zł</code></li>
        <li>Weekend dostanie: <code>30 000 / 25 × 0.5 = 600 zł</code></li>
        </ul>
        <pre><code>Target wg Dni = 
        <span class='dax-function'>SUMX</span>(
            <span class='dax-function'>VALUES</span>( dKalendarz[Date] ),
            <span class='dax-keyword'>VAR</span> CurrentDate = dKalendarz[Date]
            <span class='dax-keyword'>VAR</span> CurrentMonth = <span class='dax-function'>EOMONTH</span>( CurrentDate, <span class='dax-number'>0</span> )

            // Pobierz target dla tego miesiąca
            <span class='dax-keyword'>VAR</span> MonthlyTarget = 
                <span class='dax-function'>CALCULATE</span>(
                    <span class='dax-function'>SUM</span>( fCele[Cel] ),
                    <span class='dax-function'>FILTER</span>(
                        <span class='dax-function'>ALL</span>( dKalendarz[Date] ),
                        <span class='dax-function'>EOMONTH</span>( dKalendarz[Date], <span class='dax-number'>0</span> ) = CurrentMonth
                    )
                )

            // Policz dni w miesiącu z wagami
            <span class='dax-keyword'>VAR</span> DaysWithWeights = 
                <span class='dax-function'>SUMX</span>(
                    <span class='dax-function'>FILTER</span>(
                        <span class='dax-function'>ALL</span>( dKalendarz ),
                        <span class='dax-function'>EOMONTH</span>( dKalendarz[Date], <span class='dax-number'>0</span> ) = CurrentMonth
                    ),
                    <span class='dax-function'>IF</span>( dKalendarz[CzyDzienRoboczy] = <span class='dax-number'>1</span>, <span class='dax-number'>1</span>, <span class='dax-number'>0.5</span> )
                )

            // Waga dla bieżącego dnia (sprawdź czy ta konkretna data to dzień roboczy)
            <span class='dax-keyword'>VAR</span> CurrentDayWeight = 
                <span class='dax-keyword'>VAR</span> IsDzienRoboczy = 
                    <span class='dax-function'>CALCULATE</span>(
                        <span class='dax-function'>MAX</span>( dKalendarz[CzyDzienRoboczy] ),
                        dKalendarz[Date] = CurrentDate
                    )
                <span class='dax-keyword'>RETURN</span>
                    <span class='dax-function'>IF</span>( IsDzienRoboczy = <span class='dax-number'>1</span>, <span class='dax-number'>1</span>, <span class='dax-number'>0.5</span> )

            // Dzienna wartość targetu
            <span class='dax-keyword'>VAR</span> DailyTarget = 
                <span class='dax-function'>DIVIDE</span>( MonthlyTarget, DaysWithWeights ) * CurrentDayWeight

            <span class='dax-keyword'>RETURN</span> DailyTarget
        )</code></pre>
...
(function() {
    ...
    window['showPage_68f25e0b'] = function(n) {
        if (n > totalPages) currentPage_68f25e0b = totalPages;
        if (n < 1) currentPage_68f25e0b = 1;

        for (var i = 0; i < pages.length; i++) {
            pages[i].classList.remove('active');
        }
        pages[currentPage_68f25e0b - 1].classList.add('active');

        document.getElementById('currentPage_' + vizId).textContent = currentPage_68f25e0b;
        document.getElementById('prevBtn_' + vizId).disabled = (currentPage_68f25e0b === 1);
        document.getElementById('nextBtn_' + vizId).disabled = (currentPage_68f25e0b === totalPages);
    };

    window['changePage_68f25e0b'] = function(n) {
        currentPage_68f25e0b += n;
        window['showPage_68f25e0b'](currentPage_68f25e0b);
    };

    // Inicjalizacja
    window['showPage_68f25e0b'](1);
...
"
```
            lineageTag: 2a362e75-354b-4148-a93d-61dfd62ae838
...
<div class='container' id='viz_b364ba4a'>
    <!-- Nawigacja na górze -->
    <div class='navigation'>
        <button id='prevBtn_b364ba4a' onclick='changePage_b364ba4a(-1)'>← Poprzednia</button>
        <span class='page-indicator'>
            Strona <span id='currentPage_b364ba4a'>1</span> z <span id='totalPages_b364ba4a'>2</span>
        </span>
        <button id='nextBtn_b364ba4a' onclick='changePage_b364ba4a(1)'>Następna →</button>
    </div>

    <!-- Strona 1 -->
    <div class='page active'>
        <h1>Przewalutowanie</h1>
        <h2><strong>Komponenty rozwiązania</strong></h2>
        <p><strong>1. Tabela Waluta</strong></p>
        <p>Tabela niepołączona z modelem, zawierająca wartości walut (np. ”PLN”, ”EUR”). Użytkownik wybiera walutę przez slicer, który filtruje tę tabelę.</p>
        <p><strong>2. Numeric Range <code>_param_Euro-PLN</code></strong></p>
        <p>Pozwala użytkownikowi dynamicznie wybrać kurs wymiany. Numeric Range działa jak przełącznik między różnymi miarami lub wartościami.</p>

    </div>

    <!-- Strona 2 -->
    <div class='page'>
        <h2><strong>Jak działa miara przewalutowania?</strong></h2>
        <p>Miara <code>Wykonanie Brutto (przewalutowanie)</code>:</p>
        <ul>
        <li>Sprawdza wybraną walutę z tabeli Waluta</li>
        <li>Jeśli wybrano PLN: mnoży wartość przez kurs z field parametera (dzieli przez 100, bo kurs jest zapisany w groszach/setnych)</li>
        <li>Jeśli wybrano EUR: zwraca wartość oryginalną bez konwersji</li>
        </ul>
        <pre><code>Wykonanie Brutto (przewalutowanie) = 
        <span class='dax-keyword'>VAR</span> _waluta = <span class='dax-function'>SELECTEDVALUE</span>( Waluta[Waluta] )
        <span class='dax-keyword'>RETURN</span>
            <span class='dax-function'>IF</span> ( 
                _waluta = ”PLN”,
                [Wykonanie brutto] * ´_param_Euro-PLN´[Kurs Value] / <span class='dax-number'>100</span>,
                [Wykonanie brutto]
            )</code></pre>
        <h2><strong>Zastosowanie</strong></h2>
        <ul>
        <li>Użytkownik wybiera walutę (PLN/EUR) przez slicer</li>
        <li>Użytkownik ustawia kurs wymiany przez numeric range parameter</li>
        <li>Miara automatycznie przelicza wartości zgodnie z wyborem</li>
        </ul>
...
(function() {
    ...
    window['showPage_b364ba4a'] = function(n) {
        if (n > totalPages) currentPage_b364ba4a = totalPages;
        if (n < 1) currentPage_b364ba4a = 1;

        for (var i = 0; i < pages.length; i++) {
            pages[i].classList.remove('active');
        }
        pages[currentPage_b364ba4a - 1].classList.add('active');

        document.getElementById('currentPage_' + vizId).textContent = currentPage_b364ba4a;
        document.getElementById('prevBtn_' + vizId).disabled = (currentPage_b364ba4a === 1);
        document.getElementById('nextBtn_' + vizId).disabled = (currentPage_b364ba4a === totalPages);
    };

    window['changePage_b364ba4a'] = function(n) {
        currentPage_b364ba4a += n;
        window['showPage_b364ba4a'](currentPage_b364ba4a);
    };

    // Inicjalizacja
    window['showPage_b364ba4a'](1);
...
"
```
            lineageTag: 813a18c5-4e74-4a46-8970-1d9dab87d918
...
<div class='container' id='viz_b61f34a7'>
    ...
    <div class='page active'>
        ...
        <p>Funkcja aktywująca nieaktywne relacje w modelu danych. Działa tylko wewnątrz <code>CALCULATE()</code>.</p>
        <p><strong>Składnia</strong>:</p>
        <p><code>USERELATIONSHIP(**Kolumna1, Kolumna2)</code></p>
        <h2><strong>Problem biznesowy</strong></h2>
        <p>Tabela Sprzedaż ma wiele dat (Faktura, Sprzedaż, Dostawa), ale tylko jedno aktywne połączenie z Kalendarzem.</p>
        <p>Aktywna relacja:<code>Data Faktury → dKalendarz[Date]</code></p>
        <p>Przykład zastosowanie <code>USERELATIONSHIP</code>:</p>
        <pre><code>DAX
        Wykonanie Brutto wg Daty Sprzedaży = 
             <span class='dax-function'>CALCULATE</span>(
                 <span class='dax-function'>SUM</span>(´Sprzedaż´[Wartość Brutto]),
                 <span class='dax-function'>USERELATIONSHIP</span>(´Sprzedaż´[Data Sprzedaży], dKalendarz[Date])
             )</code></pre>
    ...
    <div class='page'>
        ...
        <p>Pamiętaj, aby uprzednio utworzyć relację z pozostałymi datami, czyli:</p>
        <p><code>Data Sprzedaży → dKalendarz[Date]</code></p>
        <p><code>Data Dostawy → dKalendarz[Date]</code></p>
        <p>Obie te relacje powinny być <strong>nieaktwyne.</strong></p>
...
"
```
            lineageTag: f953b605-e7fa-4a43-af26-6b56a83fe132
...
<head>
    <meta charset='UTF-8'>
    <meta name='viewport' content='width=device-width, initial-scale=1.0'>
    <title>Warunkowe kolorowanie</title>
    <style>
            * {
                margin: 0;
                padding: 0;
...
<div class='container' id='viz_657b8ead'>
    <!-- Nawigacja na górze -->
    <div class='navigation'>
        <button id='prevBtn_657b8ead' onclick='changePage_657b8ead(-1)'>← Poprzednia</button>
        <span class='page-indicator'>
            Strona <span id='currentPage_657b8ead'>1</span> z <span id='totalPages_657b8ead'>2</span>
        </span>
        <button id='nextBtn_657b8ead' onclick='changePage_657b8ead(1)'>Następna →</button>
    </div>

    <!-- Strona 1 -->
    <div class='page active'>
        <h1>Formatowanie warunkowe</h1>
        <p>Miara zwracająca kod koloru na podstawie porównania wartości z normą (średnią miesięczną).</p>
        <h2><strong>Zastosowanie na wykresie słupkowym</strong></h2>
        <p>W wizualizacji słupkowej Power BI możesz dynamicznie kolorować słupki na podstawie warunku biznesowego.</p>
        <p><strong>Krok 1</strong>: Utwórz miarę zwracającą kod koloru:</p>
        <pre><code>Sprzedaż powyżej normy (kolor) = 
        <span class='dax-keyword'>VAR</span> WartoscBiezacegoMiesiaca = 
            <span class='dax-function'>CALCULATE</span>( <span class='dax-function'>SUM</span>( ´Sprzedaż´[Wartość Brutto] ) )

        <span class='dax-keyword'>VAR</span> SredniaMiesieczna = 
            <span class='dax-function'>DIVIDE</span>(
                <span class='dax-function'>CALCULATE</span>(
                    <span class='dax-function'>SUM</span>( ´Sprzedaż´[Wartość Brutto] ), 
                    <span class='dax-function'>REMOVEFILTERS</span>( dKalendarz ),
                    <span class='dax-function'>VALUES</span>( dKalendarz[Rok] )
                ),
                <span class='dax-function'>CALCULATE</span>( 
                    <span class='dax-function'>DISTINCTCOUNT</span>( dKalendarz[MiesiacSkr] ),
                    <span class='dax-function'>REMOVEFILTERS</span>( dKalendarz ),
                    <span class='dax-function'>VALUES</span>( dKalendarz[Rok] )
                )
            )
    ...
    <!-- Strona 2 -->
    <div class='page'>
        <p><strong>Krok 2</strong>: W wizualizacji słupkowej przejdź do formatowania → Kolory słupków → Formatowanie warunkowe → Reguły → Wybierz miarę <code>Sprzedaż powyżej normy (kolor)</code></p>
        <p><strong>Efekt</strong>: Słupki poniżej średniej miesięcznej będą szare (#cacaca), a powyżej średniej – niebieskie (#265dd2).</p>
...
(function() {
    ...
    window['showPage_657b8ead'] = function(n) {
        if (n > totalPages) currentPage_657b8ead = totalPages;
        if (n < 1) currentPage_657b8ead = 1;

        for (var i = 0; i < pages.length; i++) {
            pages[i].classList.remove('active');
        }
        pages[currentPage_657b8ead - 1].classList.add('active');

        document.getElementById('currentPage_' + vizId).textContent = currentPage_657b8ead;
        document.getElementById('prevBtn_' + vizId).disabled = (currentPage_657b8ead === 1);
        document.getElementById('nextBtn_' + vizId).disabled = (currentPage_657b8ead === totalPages);
    };

    window['changePage_657b8ead'] = function(n) {
        currentPage_657b8ead += n;
        window['showPage_657b8ead'](currentPage_657b8ead);
    };

    // Inicjalizacja
    window['showPage_657b8ead'](1);
...
"
```
            lineageTag: 50071029-f33e-40d9-b076-49471c2dcfc3
